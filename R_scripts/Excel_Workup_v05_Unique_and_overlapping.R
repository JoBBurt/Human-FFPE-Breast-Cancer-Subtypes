...
myoutput <- "output/Output_Data/Breast_FFPE_Overlapping_Proteins_2rep_panHuman_23_0105_V01.xlsx"
...
write_overlap <- function(myQval){
  ...
  writeData(wb, sheet = mySheet, 
            ...
            startRow = 2)
  writeData(wb, sheet = mySheet, 
            x = paste("Significantly Altered Overlapping Protein Groups in All Comparisons with |log2(FC)| ≥ 0.58 & q-value ≤", myQval),
            startRow = 3)
  writeData(wb, sheet = mySheet, x = overlap, 
            startRow = 5)
}

write_unique <- function(x){
  dq <- all_comparisons[all_comparisons$Absolute.AVG.Log2.Ratio >= 0.58 & 
                          all_comparisons$Qvalue <= myQval,]
  
  df <- unique(dq[with(dq, ave(seq_along(Comparison..group1.group2.), Genes, FUN = length) == 1),])
  df <- df[df$Comparison..group1.group2. == x,]
  ...
  writeData(wb, sheet = mysheet, 
            x = paste(comma(nrow(df)), 
                      "Unique Significantly Altered Protein Groups with |log2(FC)| ≥ 0.58 & q-value ≤", myQval),
            startRow = 3)
...
if(add_q0.05 == TRUE){
  myQval <- 0.05
  write_overlap(myQval)
  sapply(unique(all_comparisons$Comparison..group1.group2.), write_unique)
}
